...
export function PnlHistory({ onOpenDay }: { onOpenDay: (idx: number) => void }) {
  ...
  return (
    <Card className="p-4">
      <div className="mb-3 flex h-[120px] items-end gap-1 overflow-x-auto pb-1">
        {hist.map((d, idx) => {
            ...
            >
              ...
              <span
                className={cn(
                  ...
                  d.total >= 0 ? "bg-live/70" : "bg-loss/70"
                )}
                style={{ height: h + "%" }}
              />
      ...
      <div className="grid grid-cols-2 gap-2 border-t border-line pt-3 font-mono text-[0.64rem] sm:grid-cols-5">
        ...
        <div>
          ...
          <div className={cn("text-[0.85rem] font-bold", totalProfit >= 0 ? "text-live" : "text-loss")}>
        ...
        <div>
          ...
          <div className="text-[0.85rem] font-bold text-payout">
            +${Math.round(totalPayouts).toLocaleString()}
          </div>
        </div>
        <div>
          <div className="text-[0.5rem] uppercase tracking-[0.1em] text-faint">Best day</div>
          <div className="text-[0.85rem] font-bold text-live">+${Math.round(best).toLocaleString()}</div>
        </div>
      </div>
